...
from src.workers.stock_worker import is_trading_day


class TestIsTradingDay:
    def test_returns_true_when_date_in_calendar(self):
        mock_df = pd.DataFrame({"trade_date": pd.to_datetime(["2026-05-29", "2026-05-28"])})
        with patch("src.workers.stock_worker.ak.tool_trade_date_hist_sina", return_value=mock_df):
    ...
    def test_returns_true_on_api_failure(self):
        with patch("src.workers.stock_worker.ak.tool_trade_date_hist_sina", side_effect=Exception("API down")):
            assert is_trading_day(date(2026, 5, 29)) is True
